...
def walk_with_snapshots(x, x_abs, D, v, dt, S, t_max):
    ...
    for _ in range(steps):
        # advance by S steps
        for _ in range(S):
            x = x + v * dt + np.sqrt(2 * D * dt) * np.random.normal(0, 1, len(x))
...
def norm_dist(x, mu, v):
    ...
    return 1 / (2 * np.pi * v) ** (1 / 2) * np.exp(-((x - mu) ** 2) / (2 * v))
...
class AnimatedLines:
    ...
    def __init__(self, ax, *data):
        """Creates an animated line with automatic style.
        ...
        self._absolute_index = self._data[0][0].shape != self._data[0][1].shape
        ...
        for i, (x_data, y_data, label) in enumerate(self._data):
            ...
            line = self.ax.plot(
                x_data, y_data, label=label, linestyle=linestyle, color=color
            )
...
def plot_walk(positions, snapshots, theo_positions, theo_snapshots, bins, axes, event):
    ...
    norm_lines = AnimatedLines(
        ax_norm, (t, norms, "Simulated Norm"), (t, t_norms, "Analytical Norm")
    )

    mean_lines = AnimatedLines(
        ax_mean, (t, means, "Simulated Mean"), (t, t_means, "Analytical Mean")
    )

    variance_lines = AnimatedLines(
        ax_var,
        (t, variances, "Simulated Variance"),
        (t, t_variances, "Analytical Varance"),
    )

    t_pos_line = AnimatedLines(
        ax_hist,
        (theo_positions[0], theo_positions[1], "Theoretical Prop. Dist. w/o Drain"),
        (theo_positions[0], theo_positions[2], "Theoretical Prop. Dist. w/ Drain",),
    )
...
def theoretical_diffusion(x0, v, t, D, limits):
    ...
    positions_abs = positions - norm_dist(
        x, 2 * x_abs - x0 + v * t, 2 * D * t
    ) * norm_dist(x_abs, x0 + v * t, 2 * D * t) / norm_dist(
        x_abs, 2 * x_abs - x0 + v * t, 2 * D * t
    )  # with absorbtion
...
def main():
    ...
    # initialize the ensemple and simulate part.
    x = np.ones(R) * x0  # we all start at x0
    positions, snapshots = walk_with_snapshots(x, limits[1], D, v, dt, S, t_max)
    theo_positions, theo_snapshots = theoretical_diffusion(
        x0, v, snapshots.T[0], D, limits
    )

    # handle clicks and show plot
    on_click = partial(
        plot_walk, positions, snapshots, theo_positions, theo_snapshots, bins, axes
    )
...
veringert somit die Norm ab ca.  t=3.8 merklich.  Der mittelwert der
Verteilung wandert ab dieser Zeit nicht mehr mit v_drift nach rechts
sondern kehrt ab t~=10 sogar nach links um.  Da die Verteilung durch
v_drift und den rand nur beding breitlaufen kann steigt auch die
Varianz immer weniger an (divergiert von theorie ohne abs.  ab t~=5).
Die Form der Warscheinlichkeitsverteilung schein sich 'anzustauen' da
nur teilchem mit geringerem impuls 'ueberleben'.  Gleichzeitig bewegen
sich teilchen in der Naehe der Wand nicht weit von selbiger fort und
werden durch die Driftgeschwindigkeit nach kurzerzeit in diese
hineinbewegt.  Somit geht auch das Histogram in Wandnaehe gegen null.
Schaltet man die Absorbtion ab folgt die Simulation sehr gut den
Theoretischen zusammenthaengen.

b) Am meisten aendert sich die Vert.  um x=x_abs als bei der
...
weniger durch die Wand beeinflusst.  Der Drift scheint die Randeffekte
zu ueberlagern.
